...
// Converted from version in 1brc java
object CreateMeasurements {
    private val MEASUREMENT_FILE: Path = Path.of("./measurements.txt")
    ...
    fun argsToSize(args: Array<String>): Int {
        if (args.size != 1) return 1_000_000_000
        val size = args[0].toIntOrNull()
        if (size == null) {
            println("Invalid value for <number of records to create>")
            println("Usage: CreateMeasurements <number of records to create>")
            exitProcess(1)
        }
        return size
    }
    ...
    @JvmRecord
    private data class WeatherStation(
        val id: String,
        val meanTemperature: Double,
    ) {
        fun measurement(): Double {
            val measurement = ThreadLocalRandom.current().nextGaussian(meanTemperature, 10.0)
            return round(measurement * 10.0) / 10.0
        }
    }
}
